...
var db = openDatabase('sistemaBylocopoet', '1.0', 'locopoet DB', 2 * 1024 * 1024);
var totalmsj=0;
function crearTabla(){
    var f=new Date();
    var dia=f.getDate();
    ...
    db.transaction(function (SQL) {
        ...
        var NICKNAME= '';		
        var PASSWORD= encriptar("locopoet",1000,anio,mes,dia); 
        var ONLINE= 'red';		
        var NIVEL=1;
        SQL.executeSql('SELECT * FROM usuarios',[], function (SQL, resultado) {
            var filas = resultado.rows.length;
            if(filas < 1){
                SQL.executeSql('INSERT INTO usuarios (ID,NICKNAME,USUARIO,NIVEL,PASSWORD,FECHA,ONLINE,BAJA) VALUES (1000, "'+ NICKNAME +'", "", "'+ NIVEL +'", "'+ PASSWORD +'","'+ FECHA +'","'+ ONLINE +'","0")');  
            }
        }, null);              
    });
    if(id_usuario > 0){
            ...
            if(filas > 0){
                ...
                SQL.executeSql('SELECT * FROM usuarios WHERE ID='+id+' AND PASSWORD = "'+password+'"',[], function (SQL, resultado){
                    ...
                    if(filas > 0){
                        ...
                        if(nombre == '' || nombre == null){nombre='locopoet';}
                        setTimeout(function(){
                            abrirChat();
                        },2000);                        
                    }
